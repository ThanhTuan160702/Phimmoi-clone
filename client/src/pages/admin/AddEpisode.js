...
import { apiCreateEpisode } from '../../apis/movie'
import Swal from 'sweetalert2'
import { FadeLoader } from 'react-spinners'
import { toast } from 'react-toastify'

const AddEpisode = ({el, cancel}) => {
  ...
  const handleAddEpisode = async() => {
    if(payload.episode === '' || payload.video === null){
      ...
      setLoading(true)
      await apiCreateEpisode(formData)
      setLoading(false)
  ...
  return (
    <div className='p-8 bg-gray-300 h-screen relative'>
      ...
      <h1 className='font-bold text-4xl mb-1'>Thêm tập phim</h1>
      ...
      <div className='flex justify-center items-center mt-2'>
        ...
        <div onClick={() => handleAddEpisode()}>
          <button className='p-3 bg-blue-500 border'>Thêm</button>
        </div>
      </div>
    </div>
  )
